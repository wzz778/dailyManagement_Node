...
function showallday() {
    $.post('http://127.0.0.1/api2/alltime',
        // { 'useraccount': account },
        function (date) {
            teamTbody.innerHTML = '';
            if (userstatus == 1) {
                for (let n = 0; n < date.length; n++) {
                    for (let m = 0; m < date[n].items.length; m++) {
                        teamTbody.innerHTML += "<tr><td class='ms'><input type='checkbox' name='team_a' value='all'></td>" +
                            "<td class='ms'>" + date[n].username + "</td>" +
                            "<td class='ml'>" + date[n].items[m].begintime + "</td>" +
                            "<td class='ml'style='display: none;'>" + date[n].items[m].rebegintime + "</td>" +
                            "<td class='ml'>" + date[n].items[m].endtime + "</td>" +
                            "<td class='ml'>" + date[n].items[m].time + "</td>" +
                            "<td class='ml'  style='display: none;'>" + date[n].items[m]._id + "</td>" +
...
function teamfind() {
    ...
    if (name == '') {
        ...
        $.post('http://127.0.0.1/api2/showkwarnbyname', { 'name': name },
            function (date) {
                for (let i in date) {
                    teamTbody.innerHTML = '';
                    let account = date[i];
